...
class Helmet {
  ...
  mouseEvent() {
    ...
    window.addEventListener('mousemove', event => {
			if(mouse.x < 4.9) {
				if(event.clientX / 100 > 4.9)
				{
					mouse.x = event.clientX / 100;
				}
			}
			else if(mouse.x > 8.6) {
				if(event.clientX / 100 < 8.6)
				{
					mouse.x = event.clientX / 100;
				}
			}
			...
			// mouse.x = event.clientX / 100;
      mouse.y = event.clientY / 500;
      this.loopRotate(mouse.x, mouse.y);
    });
	}
